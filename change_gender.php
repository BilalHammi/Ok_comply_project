...
if (isset($_POST['Submit'])) {
    ...
    @$gender2 = $_POST['genderConfirm'];
    if ($gender1 === $gender2) {
        $sql = "UPDATE `user_information` SET `gender`=:gender1 WHERE user_account_id = '" . $_SESSION['UserLoggedIn'] . "'";
        $stmt = $conectie->prepare($sql);
        $stmt->bindParam(':gender1', $gender1);
        $stmt->execute();
        $result = $stmt->fetch();
        ...
        <div class="flex justify-center items-center relative top-80">
            <div class="min-[460px]:hidden max-[450px]:visible bg-blue-400 mt-10 h-screen max-h-[22rem] w-[20rem] rounded-lg">
                <form action="change_gender.php" method="post">
                    <div class="">
                        <p class="font-bold text-lg pt-9 ml-10 text-gray-300">gender</p>
                        <input type="text" name="gender1" class="ml-10 pt-1 bg-inherit border-b-2 border-black outline-none font-bold" value=<?php echo $account_information["gender"]; ?>>
                    </div>
                    <div class="">
                        <p class="font-bold text-lg pt-9 ml-10 text-gray-300">Confirm gender</p>
                        <input type="text" name="genderConfirm" class="ml-10 pt-1 bg-inherit border-b-2 border-black outline-none font-bold">
                    </div>
                    <?php echo @$p ?>
                    <div class="flex justify-center">
