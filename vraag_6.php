...
if (isset($_POST['pijl_r_x'])) {
    if (filter_var($_POST["vraag"], FILTER_VALIDATE_EMAIL)) {
        $vraag6 = $_POST["vraag"];
        $_SESSION["question6"] = $vraag6;
        header("Location: vraag_7.php");
    } else {
        $error = "<p class='font-bold flex justify-center mt-1'>E-mail must contain at sign</p>";
    }
}
var_dump($_SESSION["question1"]);
var_dump($_SESSION["question2"]);
var_dump($_SESSION["question3"]);
var_dump($_SESSION["question4"]);
var_dump($_SESSION["question5"]);
?>
<!DOCTYPE html>
<html lang=" en">
...
<body class=" bg-gray-300">
    <div class="flex flex-col items-center relative top-14">
        ...
        <div class="bg-white rounded-lg h-[9rem] w-[17rem] mt-10">
            <form action="vraag_6.php" method="POST" class="">
                <h1 class="flex justify-center mt-2 text-sm">What is your E-mail?</h1>
                ...
                <p class="font-bold text-lg absolute bottom-3 right-22 mt-[20px] ml-[7.5rem]">6/7</p>
            </form>
        </div>
    </div>
</body>
