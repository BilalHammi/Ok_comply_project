...
if (isset($_POST['Submit'])) {
    ...
    @$adress2 = $_POST['adressConfirm'];
    if ($date1 === $date2) {
        $sql = "UPDATE `user_information` SET `adress`=:adress1 WHERE user_account_id = '" . $_SESSION['UserLoggedIn'] . "'";
            ...
            <div class="min-[460px]:hidden max-[450px]:visible bg-blue-400 mt-10 h-screen max-h-[22rem] w-[20rem] rounded-lg">
                <form action="change_adress.php" method="post">
                    <div class="">
                        ...
                        <input type="text" name="adress1" class="ml-10 pt-1 bg-inherit border-b-2 border-black outline-none font-bold" value=<?php echo $account_information["adress"]; ?>>
                    </div>
                    <div class="">
                        <p class="font-bold text-lg pt-9 ml-10 text-gray-300">Confirm Date of birth</p>
                        <input type="text" name="adressConfirm" class="ml-10 pt-1 bg-inherit border-b-2 border-black outline-none font-bold">
                    </div>
                    <?php echo @$p ?>
                    <div class="flex justify-center">
